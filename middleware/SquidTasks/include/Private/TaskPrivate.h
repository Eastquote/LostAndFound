...
public:
	...
	auto await_transform(IsStopRequested in_awaiter)
	{
		in_awaiter.SetIsStopRequested(m_taskInternal->IsStopRequested());
		return in_awaiter;
	}
	...
	{
		static_assert(Resumable == eTaskResumable::No, "Cannot await a non-copyable (resumable) Task by copy (try co_await std::move(task), co_await WeakTaskHandle(task), or co_await task.WaitUntilDone()");
		return TaskAwaiter<tTaskRet, RefType, Resumable, promise_type>(std::move(in_task));
	}
