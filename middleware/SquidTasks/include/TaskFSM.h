...
struct TransitionDebugData
{
	...
	const char* oldStateName = nullptr;
	FSM::StateId newStateId;
	const char* newStateName = nullptr;
};
...
class StateHandle
{
	using tPredicateRet = typename std::conditional<!std::is_void<tStateInput>::value, typename std::optional<tStateInput>, bool>::type;
...
public:
	...
	{
		return m_state ? m_state->Id : StateId{};
	}
...
using StateHandle = FSM::StateHandle;
...
public:
	...
	//{
	//	using StateInputT = FSM::first_argument<tStateConstructorFn>;
	//	const FSM::StateId newStateId = m_states.size();
	//	m_states.push_back(InternalStateData(in_name));
	//	auto state = std::make_shared<FSM::State<StateInputT, tStateConstructorFn>>(std::move(in_stateCtorFn), newStateId, in_name);
	//	return FSM::StateHandle<StateInputT, tStateConstructorFn>{ state };
	//}

	// Create a new FSM state
	template<typename StateInputT, typename tStateConstructorFn>
	auto State(std::string in_name, tStateConstructorFn in_stateCtorFn)
	{
		const FSM::StateId newStateId = m_states.size();
		m_states.push_back(InternalStateData(in_name));
	...
	Task<FSM::StateId> Run(tDebugStateTransitionFn in_debugStateTransitionFn = {}) const;
...
private:
	...
	std::optional<FSM::TransitionEvent> EvaluateLinks(FSM::StateId in_curStateId, bool in_isCurrentStateComplete) const;
	...
	struct InternalStateData
	{
		...
		std::vector<FSM::LinkHandle> m_outgoingLinks;
...
{
	const int32_t stateIdx = in_originState.m_state->m_stateId.idx;
	SQUID_RUNTIME_CHECK(m_states[stateIdx].m_outgoingLinks.size() == 0, "Cannot set outgoing links more than once for each state");
	...
	SQUID_RUNTIME_CHECK(numOnCompleteLinks == 0 || numOnCompleteLinks_Unconditional <= 1, "More than one unconditional OnCompleteLink() was set");

	// Set the outgoing links for the origin state
	m_states[stateIdx].m_outgoingLinks = std::move(in_outgoingLinks);
}
...
inline std::optional<FSM::TransitionEvent> TaskFSM::EvaluateLinks(FSM::StateId in_curStateId, bool in_isCurrentStateComplete) const
{
	// Determine whether to use entry links or state-specific outgoing links
	const std::vector<FSM::LinkHandle>& links = (in_curStateId.idx < m_states.size()) ? m_states[in_curStateId.idx].m_outgoingLinks : m_entryLinks;
	...
	for(const FSM::LinkHandle& link : links)
	{
		...
		{
			if(auto Result = link.EvaluateLink()) // Check if the transition to this state is valid
			{
				return Result;
			}
		}
...
inline Task<FSM::StateId> TaskFSM::Run(tDebugStateTransitionFn in_debugStateTransitionFn) const
{
	...
	{
		const std::string stateName = (curStateId.idx < m_states.size()) ? m_states[curStateId.idx].m_debugName : "";
	...
	auto DebugStateTransition = [this, in_debugStateTransitionFn](FSM::StateId in_oldStateId, FSM::StateId in_newStateId) {
		...
		{
			const std::string& oldStateName = in_oldStateId.IsValid() ? m_states[in_oldStateId.idx].m_debugName : "<ENTRY>";
			const std::string& newStateName = m_states[in_newStateId.idx].m_debugName;
			in_debugStateTransitionFn({ in_oldStateId, oldStateName.c_str(), in_newStateId, newStateName.c_str() });
		}
	};
	...
	while(true)
	{
		...
		if(std::optional<FSM::TransitionEvent> transition = EvaluateLinks(curStateId, task.IsDone()))
		{
