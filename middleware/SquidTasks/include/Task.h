...
//--- IsStopRequested Awaiter ---//
/// Awaiter class that immediately (without suspending) yields whether this task has been issued a stop request
struct IsStopRequested : public std::suspend_never
{
	void SetIsStopRequested(bool in_isStopRequested)
	{
		m_isStopRequested = in_isStopRequested;
	}
	bool await_resume()
	{
...
#define NONVOID_ONLY template <typename U = tRet, typename std::enable_if_t<!std::is_void<U>::value>* = nullptr>
	...
	{
		static_assert(IsCopyable(), "Cannot call CancelIf() on an lvalue (try std::move(task).CancelIf())");
		return CancelTaskIf(std::move(*this), in_cancelFn);
	}
	auto CancelIfStopRequested() & /// @private Illegal lvalue implementation
	{
		static_assert(!std::is_lvalue_reference<decltype(this)>::value, "Cannot call CancelIfStopRequested() on an lvalue (try std::move(task).CancelIfStopRequested())");
		return std::move(*this).CancelIf([this] { return IsStopRequested(); });
	}
	...
	{
		static_assert(!std::is_lvalue_reference<decltype(this)>::value, "Cannot call StopIf() on an lvalue (try std::move(task).StopIf())");
		return StopTaskIf(std::move(*this), in_cancelFn);
	}
