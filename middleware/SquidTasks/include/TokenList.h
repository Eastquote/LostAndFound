...
{
	return std::make_shared<Token>(in_name);
}
...
{
	return std::make_shared<DataToken<tData>>(in_name, in_data);
}

/// @brief Container for tracking decentralized state across multiple tasks. (See \ref TokenList for more info...)
...
public:
	...
	{
		return std::make_shared<Token>(std::move(in_name), in_data);
	}
	...
	{
		return AddTokenInternal(MakeToken(std::move(in_name), in_data));
	}
	...
	std::optional<T> GetMin() const
	{
		...
		SanitizeAndProcessData([&ret](const T& in_data) {
			if(!ret || in_data < ret.value()) { ret = in_data; }
		});
		return ret;
	}
